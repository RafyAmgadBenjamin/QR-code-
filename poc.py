...
def cal_late_time(base_time, attendee_time):
    attendee_time_only = datetime.datetime.strptime( attendee_time , "%Y.%m.%d  %X").time()
...
def read_original_sheet_panda(new_attendance_time):
    ...
    for index, row in data.iterrows():
        index_attendee = check_name_exists(row[original_names_index],new_attendance_names)
        if index_attendee!=-1:
            #adding present attendace
            data.loc[index,original_new_week_name] = 1
            #adding late time
            data.loc[index,original_new_time_name] = new_attendance_time[index_attendee]
        else :
    ...
    with open('/home/rafy/Downloads/POC QR code/attendance1.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                new_attendance_late_time.append(cal_late_time("22:09:23",date_time))
                line_count += 1
